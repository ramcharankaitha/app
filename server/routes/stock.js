...
router.post('/out', async (req, res) => {
  ...
  try {
    ...
    const isLowStock = minQty > 0 && parseInt(updatedProduct.current_quantity) < minQty;
...
// Check all low stock items, send notifications, and auto-generate purchase orders where possible
router.post('/check-low-stock', async (req, res) => {
  try {
    // Get ALL products with low stock (current_quantity strictly below minimum_quantity)
    // No supplier filter — every low stock product gets a notification
    ...
    for (const product of lowStockProducts) {
      try {
        // Always send a low stock notification for every product below minimum
        const hasExistingNotif = await pool.query(
        ...
        if (hasExistingNotif.rows.length === 0) {
          const hasSupplier = product.supplier_name && product.supplier_name.trim() !== '';
          await createCriticalAlert(
            'warning',
            'Low Stock Alert',
            `"${product.product_name}" (${product.item_code}) — Current: ${product.current_quantity}, Min: ${product.minimum_quantity}${hasSupplier ? '' : '. No supplier assigned — please assign a supplier.'}`,
            product.id
          );
          notifiedProducts.push({ product: product.product_name, itemCode: product.item_code });
        }

        // Auto-generate PO only if product has a supplier
        if (!product.supplier_name || product.supplier_name.trim() === '') {
          skippedPOs.push({ product: product.product_name, itemCode: product.item_code, reason: 'No supplier assigned' });
          continue;
        }
    ...
    res.json({
      success: true,
      message: total === 0
        ? 'All products are above their minimum quantity. No low stock found.'
        : `Found ${total} low stock product${total !== 1 ? 's' : ''}. Notified admin and all supervisors. Generated ${generatedPOs.length} purchase order${generatedPOs.length !== 1 ? 's' : ''}.`,
      notifiedProducts,
      generatedPOs,
      skippedPOs,
      totalChecked: total,
      totalNotified: notifiedProducts.length,
