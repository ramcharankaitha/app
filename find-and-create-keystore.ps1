...
Write-Host "`n=== Finding keytool for Android Keystore Creation ===" -ForegroundColor Green
Write-Host ""
...
foreach ($path in $keytoolPaths) {
    if ($path -like "*\*") {
        ...
        $matches = Get-ChildItem -Path (Split-Path $path -Parent) -Recurse -Filter "keytool.exe" -ErrorAction SilentlyContinue | Select-Object -First 1
        if ($matches) {
            $keytool = $matches.FullName
            break
        }
    } else {
        if (Test-Path $path) {
...
if (-not $keytool) {
    try {
        ...
        if (Test-Path $keytoolPath) {
            ...
            # Try parent directory (bin -> jre/jdk)
...
if (-not $keytool) {
    ...
    Write-Host ""
    Write-Host "Option 3: Use Online Keystore Generator" -ForegroundColor Cyan
    Write-Host "  Search for 'online keystore generator' (less secure)" -ForegroundColor White
    Write-Host ""
    exit 1
}
...
Write-Host '  4. Organizational unit (e.g., IT Department)' -ForegroundColor White
Write-Host '  5. Organization (e.g., Anitha Stores)' -ForegroundColor White
Write-Host "  6. City or Locality" -ForegroundColor White
Write-Host "  7. State or Province" -ForegroundColor White
Write-Host '  8. Two-letter country code (e.g., IN for India, US for USA)' -ForegroundColor White
