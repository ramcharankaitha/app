...
const SalesReportContent = ({ onClose }) => {
  ...
  const fetchSales = async () => {
    ...
    try {
      ...
      if (response.success && response.sales) {
        ...
        // Always set topPerformers, even if empty array
        setTopPerformers(response.topPerformers || []);
      } else {
        setError('Failed to fetch sales data');
        setTopPerformers([]); // Reset on error
      }
    } catch (err) {
              ...
              <div key={index} style={{
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: '12px', color: '#666', marginBottom: '4px' }}>
                    {formatCurrency(performer.total_revenue || 0)} • {performer.total_sales || 0} sales
                  </div>
                  {performer.user_type && (
                    <div>
