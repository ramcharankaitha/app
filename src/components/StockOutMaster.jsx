...
const StockOutMaster = ({ onBack, onAddStockOut, onNavigate, userRole = 'admin' }) => {
  ...
  const handleVerifyTransaction = async (transaction) => {
    ...
    try {
      setError('');
      const response = await stockAPI.verifyStockOut(transaction.id);
      if (response.success) {
            ...
            {isLoading ? (
              ...
              <div style={{ textAlign: 'center', padding: '40px' }}>
                ...
                <p style={{ color: '#666', fontSize: '16px' }}>
                  {searchQuery ? 'No transactions found matching your search.' : 'No stock out transactions yet.'}
                </p>
                {!searchQuery && (
                  <button className="add-btn" onClick={handleAddStockOut} style={{ marginTop: '16px' }}>
                    <i className="fas fa-plus"></i>
                    <span>Create First Stock Out</span>
                  </button>
                )}
              </div>
            ) : (
        ...
        >
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <h2 style={{ margin: 0, color: 'var(--text-primary)' }}>Stock Out Transaction Details</h2>
              <button
                onClick={closeViewModal}
                style={{
                  background: 'none',
            ...
            <div style={{ display: 'grid', gap: '16px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
                ...
                <div style={{ padding: '12px', background: '#f8d7da', borderRadius: '8px' }}>
                  <strong>Quantity Removed:</strong> <span style={{ color: '#dc3545', fontWeight: 'bold' }}>-{selectedTransaction.quantity || 0}</span>
                </div>
              ...
              {(userRole === 'admin' || userRole === 'supervisor') && (
                <div style={{ padding: '12px', background: '#f8f9fa', borderRadius: '8px', marginTop: '16px', borderTop: '1px solid #e0e0e0', paddingTop: '16px' }}>
                  {selectedTransaction.is_verified !== true ? (
                    <button
                      onClick={async () => {
                        try {
